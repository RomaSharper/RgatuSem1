...
def check_format(text: str) -> None:
    ...
    if not bool(text) or not all(['a' <= char <= 'z' for char in text]):
...
def get_prog_matrix(items: List[str]) -> List[List[str]]:
    first_letters = set(item[0] for item in items)
...
if __name__ == "__main__":
    try:
        user_count = input()
        if not user_count.isnumeric():
            raise ValueError(WRONG_NUMBER_MSG.format(user_count))
        user_count = int(user_count)
        if not MIN_COUNT <= user_count <= MAX_COUNT:
            raise ValueError(WRONG_RANGE_MSG.format(user_count))
